...
def validate(epoch, model, _, loss_function, val_loader, writer):
    model.eval()
    torch.no_grad()
...
def train(epoch, model, optimizer, loss_function, train_loader, writer):
    ...
    for step, (images, poses, gazes) in enumerate(train_loader):
        ...
        if step % 25 == 0:
            ...
            print("Train: now in epoch " + str(epoch) + " and step number " + str(step) +
                  " loss is " + str(loss.item()) + " angle error is: " + str(angle_error.item()))
